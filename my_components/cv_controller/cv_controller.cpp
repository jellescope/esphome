...
void CVController::update_status() {
  ...
  switch (cv_status_) {
    ...
    case START:
      ...
      if (selected_device_index_ == 0 && ha_temperature_devices_[selected_device_index_].deltaTemperature() > 3.0) {
        // selected device = kamer muur thermostate
        // and the temperature difference is large enough to start an adaption cycle
        cv_status_ = ADAPT;
    ...
    case NORMAL:
      ...
      control_temperature_ = max_temperature_ * control_factor;
      if (debug_) {
        ESP_LOGI(TAG, "Control_factor: %0.1f", control_factor);
        ESP_LOGI(TAG, "Control_temperature %0.1f", control_temperature_);
      }
      break;
    default:
      break;
  ...
  if (this->sensor_control_temperature_ != nullptr) {
    float publis_temperature = control_temperature_;
    if (publis_temperature < 25.0) {
      publis_temperature = 0.0;  // otherwise the remeha will start non efficient burn cycles each 10 minutes
    }
    this->sensor_control_temperature_->publish_state(publis_temperature);
  }
